...
// Thời gian tồn tại của cookie (7 ngày)
...
// Nếu phương thức POST được sử dụng để gửi form
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Nếu không có lỗi, lưu thông tin vào cookie và hiển thị kết quả
    if (empty($errors)) {
        // Lưu thông tin vào cookie
        setcookie('first_name', $first_name, $cookie_expiry);
        setcookie('last_name', $last_name, $cookie_expiry);
        setcookie('email', $email, $cookie_expiry);
        setcookie('invoice_id', $invoice_id, $cookie_expiry);
        setcookie('pay_for', json_encode($pay_for), $cookie_expiry); // Lưu mảng dưới dạng JSON
        setcookie('additional_info', $additional_info, $cookie_expiry);
        setcookie('file_upload', $file_upload, $cookie_expiry);
    ...
    if (isset($_COOKIE['first_name'])) {
        ...
        $pay_for = json_decode($_COOKIE['pay_for'], true); // Chuyển đổi JSON thành mảng
        $additional_info = $_COOKIE['additional_info'];
